...
def test_shape_self_collide_single_contact(shape1,
                                           shape2,
                                           tf1,
                                           tf2,
                                           normal,
    ...
    np.testing.assert_allclose(contact.normal,
                               normal,
                               rtol=0,
                               atol=normal_atol + double_float_difference)
    np.testing.assert_allclose(contact.pos,
                               pos,
                               rtol=0,
                               atol=pos_atol + double_float_difference)
    np.testing.assert_allclose(contact.penetration_depth,
                               penetration_depth,
                               rtol=0,
                               atol=depth_atol + double_float_difference)


def test_shape_self_distance(shape1,
                             shape2,
                             tf1,
                             tf2,
                             expected_distance,
...
class TestTriangleP(unittest.TestCase):
    def test_properties(self):
        ...
        np.testing.assert_allclose(tri.a,
                                   random_vertices[0],
                                   rtol=0,
                                   atol=0 + double_float_difference)
        np.testing.assert_allclose(tri.b,
                                   random_vertices[1],
                                   rtol=0,
                                   atol=0 + double_float_difference)
        np.testing.assert_allclose(tri.c,
                                   random_vertices[2],
                                   rtol=0,
                                   atol=0 + double_float_difference)
...
class TestBox(unittest.TestCase):
    def test_properties(self):
        ...
        np.testing.assert_allclose(box.side,
                                   random_sides,
                                   rtol=0,
                                   atol=0 + double_float_difference)
    ...
    def test_self_collide_with_contact(self):
        # Contact on X axis
        test_shape_self_collide_single_contact(
            fcl.Box(1, 1, 1),
            fcl.Box(1, 2, 3),
            fcl.Transform(np.array([1, 0, 0, 0]), np.array([0, 0, 0])),
    ...
    def test_self_distance(self):
        ...
        test_shape_self_distance(
            fcl.Box(1, 1, 1), fcl.Box(1, 2, 3),
            fcl.Transform(np.array([1, 0, 0, 0]), np.array([0, 0, 0])),
            fcl.Transform(np.array([1, 0, 0, 0]), np.array([2, 0, 0])), 1.0)

        test_shape_self_distance(
            fcl.Box(1, 1, 1), fcl.Box(1, 2, 3),
            fcl.Transform(np.array([1, 0, 0, 0]), np.array([0, 0, 0])),
            fcl.Transform(np.array([1, 0, 0, 0]), np.array([0, 3, 0])), 1.5)

        test_shape_self_distance(
            fcl.Box(1, 1, 1), fcl.Box(1, 2, 3),
            fcl.Transform(np.array([1, 0, 0, 0]), np.array([0, 0, 0])),
            fcl.Transform(np.array([1, 0, 0, 0]), np.array([0, 0, 3])), 1.0)

        # If two boxes collide, distance() should return -1.
        test_shape_self_distance(
            fcl.Box(1, 1, 1), fcl.Box(1, 2, 3),
            fcl.Transform(np.array([1, 0, 0, 0]), np.array([0, 0, 0])),
            fcl.Transform(np.array([1, 0, 0, 0]), np.array([1, 0, 0])), -1.0)
...
class TestSphere(unittest.TestCase):
    def test_properties(self):
        ...
        np.testing.assert_allclose(sphere.radius,
                                   random_radius,
                                   rtol=0,
                                   atol=0 + double_float_difference)
    ...
    def test_self_distance(self):
        test_shape_self_distance(
            fcl.Sphere(1), fcl.Sphere(2),
            fcl.Transform(np.array([1, 0, 0, 0]), np.array([0, 0, 0])),
            fcl.Transform(np.array([1, 0, 0, 0]), np.array([4, 0, 0])), 1.)

        test_shape_self_distance(
            fcl.Sphere(1), fcl.Sphere(2),
            fcl.Transform(np.array([1, 0, 0, 0]), np.array([0, 0, 0])),
            fcl.Transform(np.array([1, 0, 0, 0]), np.array([2, 0, 0])), -1)


class TestEllipsoid(unittest.TestCase):
    def test_properties(self):
        random_radii = np.random.rand(3)
        ellipsoid = fcl.Ellipsoid(random_radii[0], random_radii[1],
                                  random_radii[2])
        np.testing.assert_allclose(ellipsoid.radii,
                                   random_radii,
                                   rtol=0,
                                   atol=0 + double_float_difference)
    ...
    def test_self_collide(self):
        ...
        test_shape_self_collide(
            e1, e2, fcl.Transform(np.array([1, 0, 0, 0]), np.array([0, 0, 0])),
    ...
    def test_self_distance(self):
        ...
        test_shape_self_distance(e1,
                                 e2,
                                 fcl.Transform(np.array([1, 0, 0, 0]),
                                               np.array([0, 0, 0])),
                                 fcl.Transform(np.array([1, 0, 0, 0]),
                                               np.array([3, 0, 0])),
                                 1.,
        ...
        test_shape_self_distance(
            e1, e2, fcl.Transform(np.array([1, 0, 0, 0]), np.array([0, 0, 0])),
            fcl.Transform(np.array([1, 0, 0, 0]), np.array([1, 0, 0])), -1)
...
class TestCapsule(unittest.TestCase):
    def test_properties(self):
        ...
        np.testing.assert_allclose(c.radius,
                                   random_radius,
                                   rtol=0,
                                   atol=0 + double_float_difference)
        np.testing.assert_allclose(c.lz,
                                   random_lz,
                                   rtol=0,
                                   atol=0 + double_float_difference)
    ...
    def test_self_distance(self):
        ...
        test_shape_self_distance(
            c1, c2, fcl.Transform(np.array([1, 0, 0, 0]), np.array([0, 0, 0])),
            fcl.Transform(np.array([1, 0, 0, 0]), np.array([2, 0, 0])), 0.5)
        test_shape_self_distance(
            c1, c2, fcl.Transform(np.array([1, 0, 0, 0]), np.array([0, 0, 0])),
            fcl.Transform(np.array([1, 0, 0, 0]), np.array([0, 2, 0])), 0.5)
        test_shape_self_distance(
            c1, c2, fcl.Transform(np.array([1, 0, 0, 0]), np.array([0, 0, 0])),
            fcl.Transform(np.array([1, 0, 0, 0]), np.array([0, 0, 5])), 0.5)
        ...
        test_shape_self_distance(
            c1, c2, fcl.Transform(np.array([1, 0, 0, 0]), np.array([0, 0, 0])),
            fcl.Transform(np.array([1, 0, 0, 0]), np.array([0, 0, 4])), -0.5)
...
class TestCone(unittest.TestCase):
    ...
    def test_self_collide(self):
        ...
        test_shape_self_collide(
            c1, c2, fcl.Transform(np.array([1, 0, 0, 0]), np.array([0, 0, 1])),
            fcl.Transform(rotate_around_x_180_degrees, np.array([0, 0.999,
                                                                 1])), True)
        test_shape_self_collide(
            c1, c2, fcl.Transform(np.array([1, 0, 0, 0]), np.array([0, 0, 1])),
            fcl.Transform(rotate_around_x_180_degrees, np.array([0, 1.001,
                                                                 1])), False)
    ...
    def test_self_distance(self):
        ...
        test_shape_self_distance(
            c1, c2, fcl.Transform(np.array([1, 0, 0, 0]), np.array([0, 0, 0])),
            fcl.Transform(np.array([1, 0, 0, 0]), np.array([4, 0, 1])), 1.)
        test_shape_self_distance(
            c1, c2, fcl.Transform(np.array([1, 0, 0, 0]), np.array([0, 0, 0])),
            fcl.Transform(np.array([1, 0, 0, 0]), np.array([0, 4, 1])), 1.,
            1e-6)
        test_shape_self_distance(
            c1, c2, fcl.Transform(np.array([1, 0, 0, 0]), np.array([0, 0, 0])),
            fcl.Transform(np.array([1, 0, 0, 0]), np.array([0, 0, 4])), 1.)
...
class TestCylinder(unittest.TestCase):
    ...
    def test_self_distance(self):
        ...
        test_shape_self_distance(
            c1, c2, fcl.Transform(np.array([1, 0, 0, 0]), np.array([0, 0, 0])),
            fcl.Transform(np.array([1, 0, 0, 0]), np.array([3, 0, 1])), 0.5)
        test_shape_self_distance(
            c1, c2, fcl.Transform(np.array([1, 0, 0, 0]), np.array([0, 0, 0])),
            fcl.Transform(np.array([1, 0, 0, 0]), np.array([0, 3.5, 1])), 1.0,
            1e-5)
        test_shape_self_distance(
            c1, c2, fcl.Transform(np.array([1, 0, 0, 0]), np.array([0, 0, 0])),
            fcl.Transform(np.array([1, 0, 0, 0]), np.array([0, 0, 3])), 0.5)
...
class TestConvex(unittest.TestCase):
    ...
    def setUp(self):
        ...
        c1 = fcl.Convex(vertices, len(faces), cfaces)
        self.tetrahedron1 = c1

        vertices = np.array([[0, 0, 1], [0, 0, 0], [-1, 0, 0], [0, 1, 0]])
        c2 = fcl.Convex(vertices, len(faces), cfaces)
    ...
    def test_self_distance(self):
        ...
        test_shape_self_distance(
            c1, c2, fcl.Transform(np.array([1, 0, 0, 0]), np.array([0, 0, 0])),
            fcl.Transform(np.array([1, 0, 0, 0]), np.array([0.001, 0, 0])), -1)
    ...
    def test_properties(self):
        ...
        c = fcl.Convex(vertices, len(faces), cfaces)
...
class TestHalfspace(unittest.TestCase):
    def test_properties(self):
        ...
        np.testing.assert_allclose(h.n,
                                   random_normal,
                                   rtol=0,
                                   atol=1e-15 + double_float_difference)
    ...
    def test_self_collide(self):
        # h1, h2 = fcl.Halfspace(np.array([0, 0, 1]),
        #                        -1), fcl.Halfspace(np.array([0, 0, -1]), 2)
...
class TestBVHModel(unittest.TestCase):
    ...
    def setUp(self):
        ...
        vertices = np.array([[0., 0., 0.], [0., 0., 3.], [0., 2., 0.],
                             [0., 2., 3.], [1., 0., 0.], [1., 0., 3.],
                             [1., 2., 0.], [1., 2., 3.]])
    ...
    def test_mesh_self_distance(self):
        ...
        test_shape_self_distance(
            self.box_mesh, self.box_mesh,
            fcl.Transform(np.array([1, 0, 0, 0]), np.array([0, 0, 0])),
            fcl.Transform(np.array([1, 0, 0, 0]), np.array([-0.999, 0, 0])), 0)
        test_shape_self_distance(self.box_mesh,
                                 self.box_mesh,
                                 fcl.Transform(np.array([1, 0, 0, 0]),
                                               np.array([0, 0, 0])),
                                 fcl.Transform(np.array([1, 0, 0, 0]),
